...
import {
  Users, UserCheck, MapPin, Shield, Headphones, Sparkles, BadgeIndianRupee,
  Wrench, Plane, Building2, Heart, Mountain, Users2, Crown, PhoneCall,
  MessageCircle, ArrowRight, Filter, KeyRound,
} from "lucide-react";
...
function VehiclesPage() {
  ...
  const filtered = useMemo(() => {
    let list = activeCat === "all" ? [...VEHICLES] : VEHICLES.filter((v) => v.category === activeCat);
  ...
  return (
    <PageLayout>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 max-w-5xl">
          {stats.map((s, i) => (
            ...
            >
              ...
              <div className="text-[10px] uppercase tracking-[0.22em] text-luxury-gray mt-1">{s.label}</div>
            </motion.div>
          ))}
        </div>
      </PageHero>
      ...
      <section id="tempo-traveller" className="px-6 py-10 md:py-14">
        <div className="mx-auto max-w-7xl">
          <SectionTitle eyebrow={`${activeCat === "all" ? "Full Fleet" : CATEGORY_BY_ID[activeCat].name}`} title={`${filtered.length} vehicles available.`}>
            {activeCat === "all" ? "Tap any category above to narrow down, or pick a rental type to see matching prices." : CATEGORY_BY_ID[activeCat].blurb}
          </SectionTitle>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
            {useCases.map((u, i) => (
              ...
              >
                <span className="grid place-items-center w-12 h-12 rounded-full mx-auto mb-3" style={{ background: "var(--gradient-gold)" }}>
      ...
      <section className="px-6 py-16 md:py-20">
        <div className="mx-auto max-w-6xl relative rounded-3xl overflow-hidden border border-[var(--gold)]/30" style={{ background: "linear-gradient(135deg, var(--deep-2) 0%, oklch(0.16 0.02 260) 60%, oklch(0.22 0.05 80 / 0.4) 100%)" }}>
          <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full opacity-40" style={{ background: "var(--gradient-radial-gold)" }} />
          <div className="relative p-10 md:p-16 grid md:grid-cols-2 gap-8 items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:justify-end">
              <a href={BRAND.phoneHref} className="btn-gold px-6 py-3.5 rounded-full text-xs uppercase tracking-[0.22em] inline-flex items-center justify-center gap-2">
                <PhoneCall className="w-4 h-4" /> Call Now
              </a>
              <a href={BRAND.whatsappHref} target="_blank" rel="noreferrer" className="btn-ghost-luxe px-6 py-3.5 rounded-full text-xs uppercase tracking-[0.22em] inline-flex items-center justify-center gap-2">
                <MessageCircle className="w-4 h-4" /> WhatsApp Us
              </a>
              <Link to="/inquiry" search={{ type: "Vehicle Inquiry" }} className="btn-ghost-luxe px-6 py-3.5 rounded-full text-xs uppercase tracking-[0.22em] inline-flex items-center justify-center gap-2">
  ...
  return (
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-b from-white/5 to-transparent">
        <img src={v.image} alt={v.name} loading="lazy" width={1280} height={800} className="w-full h-full object-contain transition-transform duration-700 hover:scale-105" />
        {v.popular && (
          <div className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] uppercase tracking-[0.22em] btn-gold">Popular</div>
        )}
      </div>
      <div className="p-5">
        ...
        <div className="mt-3 space-y-0.5">
          {isLuxury ? (
            <div className="text-gold font-display text-lg leading-tight">{priceFor(v, "local")}</div>
          ) : (
            <>
        ...
        {v.notes && v.notes.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {v.notes.map((n) => (
              <span key={n} className="px-2 py-1 rounded-full glass text-[10px] text-luxury-gray">{n}</span>
            ))}
          </div>
        )}
